...
class SqlAlchemyRefreshTokenRepo:
    ...
    async def get_by_jti(
        session: AsyncSession, getter: RefreshTokenJtiGetter
    ) -> RefreshToken:
    ...
    async def get_by_user_id(
        session: AsyncSession, getter: RefreshTokenUserIdGetter
    ) -> list[RefreshToken]:
    ...
    async def update_refresh_token(
        session: AsyncSession, getter: RefreshTokenJtiGetter, values: dict[str, Any]
    ) -> RefreshToken:
        stmt = (
            update(RefreshToken)
        ...
        refresh_token = await session.scalar(stmt)
